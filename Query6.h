...
#include "include/kernel_files/selection_block.h"
#include "include/kernel_files/selection_coalesced.h"
#include "include/kernel_files/logical_block.h"
...
void test_scalar_q6(uint32_t** data,uint32_t tuples){
    ...
    start = clock();
    for(int i=0;i<tuples;i++){
        date[i] = (date[i]>0&&date[i]);
        discount[i] = (discount>0&&discount[i]<50);
        quantity[i] = (quantity[i]<24);
    }
    time_t end = clock();
    ...
    int sum = 0;
    for(int i=0;i<tuples;i++){
        sum = sum + (extended_price[i]*discount[i]*(date[i]*discount[i]*quantity[i]));
    }
    end = clock();
    cout<<"Scalar execution time (in Secs): "<<(double)(end - start)/CLOCKS_PER_SEC<<endl;
...
void cross_device(){
    ...
    //Ship date and discount are selection between
    //quantity is selection less than

    selection_kernel_coal between("between");
...
//    logical_kernel AND("AND");
    ...
    //Execute the kernels
    execute(GPU,between.get_kernel_name(),arguments1,parameter1,between.global_size);

    less_than.set_global_size(tuples);
    execute(CPU,less_than.get_kernel_name(),arguments3,parameter3,tuples/32);

    //Combine results of all in GPU

    //Copy result from cpu to gpu
    transfer_data("selection_result3",CPU,GPU,tuples);

    //Compile AND kernel in GPU
    logical_kernel and_kernel("and");
    add_kernel(and_kernel.get_kernel_name(),GPU,and_kernel.kernel_src);

    //Allocate data
    delete_data("ship date",GPU);
    delete_data("quantity",CPU);

    int result4[tuples/32];
    int result5[tuples/32];

    add_data("selection_result4",result4,GPU,tuples/32);
    add_data("selection_result5",result5,GPU,tuples/32);
    ...
    execute(GPU,and_kernel.get_kernel_name(),arguments1,parameter1,tuples);
    arguments1.clear();

    arguments1.push_back("selection_result4");
    arguments1.push_back("selection_result3");
    arguments1.push_back("selection_result5");

    execute(GPU,and_kernel.get_kernel_name(),arguments1,parameter1,tuples);
    arguments1.clear();

    print_data("selection_result5",GPU,tuples,true);
}