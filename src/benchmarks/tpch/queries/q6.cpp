...
//   auto l_shipdate_col = rel["l_shipdate"].data<int>();
//   auto l_quantity_col = rel["l_quantity"].data<int>();
//   auto l_extendedprice_col = rel["l_extendedprice"].data<int>();
//   auto l_discount_col = rel["l_discount"].data<int>();

   revenue = tbb::parallel_reduce(
       tbb::blocked_range<size_t>(0, rel.nrTuples), types::Numeric<12, 4>(0),
       [&](const tbb::blocked_range<size_t>& r,
           const types::Numeric<12, 4>& s) {
//           tbb::blocked_range<size_t>(0, rel.nrTuples), int(0),
//           [&](const tbb::blocked_range<size_t>& r,
//            const int& s) {
          auto revenue = s;
...
unique_ptr<Q6Builder::Q6> Q6Builder::getQuery() {
   ...
   //Column value multiplication done here
...
//   auto& r = result["revenue"];
   auto& r = result["revenue"].typedAccess<int>();
