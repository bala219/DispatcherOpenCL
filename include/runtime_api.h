...
template<typename T>
void execute(cl_device_id device, string kernel_name, vector<string> args, vector<T> param,
...
//        cout<<param[i];
    ...
    clGetEventProfilingInfo(evt, cpiS, sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(evt, cpiE, sizeof(time_end), &time_end, NULL);

    execution_time_ns = time_end-time_start;
}