...
class QSimpleBuilder : public vectorwise::QueryBuilder {
...
public:
    ...
    std::unique_ptr<QSimple> getQuery();
    QSimpleBuilder(runtime::Database& db, vectorwise::SharedStateManager& shared,
              size_t size = 1024)
            : QueryBuilder(db, shared, size) {}
};

runtime::Relation
qSimple_vectorwise(runtime::Database& db,
              size_t nrThreads = std::thread::hardware_concurrency(),
              size_t vectorSize = 1024);
