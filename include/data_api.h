...
void print_data(string name, cl_device_id device, size_t global_size, bool flag_bits = false) {
    ...
    err = clEnqueueReadBuffer(queue[device].queue, argument_buffer, CL_TRUE, 0, global_size * sizeof(unsigned int),
                              result, 0, NULL, NULL);
...
template<typename T>
void read_data(string name, cl_device_id device, T *data, size_t size) {
...
void transfer_data(string name, cl_device_id source, cl_device_id target, size_t size) {
    ...
    cl_int err = clEnqueueReadBuffer(queue[source].queue, argument_buffer, CL_TRUE, 0, size * sizeof(unsigned int),
                                     result, 0, NULL, NULL);
