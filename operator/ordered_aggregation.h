...
void test_prefix_sum(){
    ...
    //Iterator to test prefixsum
    double PS_TIME = 0;
    int ps_result = 0;
    for(int i=0;i<itera;i++){
        ps_result += compute_prefix_sum(1);
        PS_TIME += ps_total;
    }
    cout<<"Total time : "<<PS_TIME/itera<<endl;
    cout<<"PS result : "<<ps_result/itera<<endl;
}
...
//This calls the run command in python
void dummyPythonCalls(short context){

}


double ExecuteVariant(short variant, short distribution, size_t group, size_t data_size, short dev, short num_of_iterations = 100){
...
void runTestingBandit(){

    short initialVariant = 0;
    short distributionValue=0;
    size_t group = pow(2,10),dataSize = pow(2,20);
    size_t iterations = 100;
    //For altering groupsizes and fixing the data size

    dummyPythonCalls(group/dataSize);

    double runTime = ExecuteVariant(initialVariant,distributionValue,group,dataSize,0,iterations);
